...
def backbone(images, reuse, backbone_type="alexnet"):
    ...
    with tf.compat.v1.variable_scope('AlexNet', reuse=tf.compat.v1.AUTO_REUSE):
...
def illuminant_estimator(features, is_training, reuse):
    """ Scope names are hard-coded for loading pretrained weights.
    """
    with tf.compat.v1.variable_scope('branch', reuse=reuse):
        ...
        illums = tf.reduce_sum(input_tensor=illum_maps, axis=(1, 2)) # RGB. (b, 3)
    return illums
...
def model(images, is_training, reuse, backbone_type='alexnet'):
    ...
    with tf.compat.v1.variable_scope("FCN", reuse=reuse):
