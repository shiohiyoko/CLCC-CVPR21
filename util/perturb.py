...
def rand_intensity_gain(image, do_prob, gain_range):
    def _do():
        rand_gain = tf.random.uniform(shape=(), minval=gain_range[0], maxval=gain_range[1], dtype=tf.float32)        
        intensity_gained_image = image * rand_gain
        return intensity_gained_image, rand_gain
    
    rand_prob = tf.random.uniform(shape=(), dtype=tf.float32)
    intensity_gained_image, intensity_gain = tf.cond(pred=tf.less(rand_prob, do_prob), true_fn=_do, false_fn=lambda: (image, 1.0))
...
def rand_shot_noise(image, do_prob, std_range):
    def _do():
        ...
        rand_std = tf.random.uniform(shape=(), minval=std_range[0], maxval=std_range[1], dtype=tf.float32)
        noise = tf.random.normal(shape=tf.shape(input=image), mean=0.0, stddev=rand_std, dtype=tf.float32)
        normalized_noisy_image = normalized_image + tf.sqrt(normalized_image) * noise
        noisy_image = normalized_noisy_image * 65535.
        return noisy_image
    
    rand_prob = tf.random.uniform(shape=(), dtype=tf.float32)
    noisy_image = tf.cond(pred=tf.less(rand_prob, do_prob), true_fn=_do, false_fn=lambda: image)
