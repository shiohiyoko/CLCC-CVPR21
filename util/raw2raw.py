...
def mask_paired_cc24s(cc24s_A, cc24s_B):
  ...
  valid_pixel_count = tf.reduce_sum(input_tensor=cc24s_mask, axis=-1) # (b,)
  norm_cc24s_A = (tf.reduce_sum(input_tensor=cc24s_A, axis=[1,2]) + EPS) / (valid_pixel_count + EPS) # (b,)
  norm_cc24s_B = (tf.reduce_sum(input_tensor=cc24s_B, axis=[1,2]) + EPS) / (valid_pixel_count + EPS) # (b,)
...
def coloraug(images, illums, cc24s, shift_id=1, w=None):
  ...
  masks_A = tf.cast(tf.cast(tf.reduce_sum(input_tensor=cc24s, axis=(1,2)), dtype=tf.bool), dtype=tf.float32)
  ...
  # -----------------------------------------------------------------------------
  b = tf.shape(input=images)[0]
  # C == A when w == 0; # C == B when w == 1.
  # C can be A when C and A has different perturbation augmentation. 
  if not w:
    w = tf.cond(pred=tf.greater_equal(tf.random.uniform(()), 0.5), # Avoid sample [-0.3, 0.3]
                true_fn=lambda: tf.random.uniform((b,), -5.0, -0.3),
                false_fn=lambda: tf.random.uniform((b,), 0.3, 5.0)) + EPS # EPS prevents nan when encountering zero cc24.
  else:
    w = tf.ones((b,)) * w
  ...
  illums_C = (illums_C + EPS) / (tf.norm(tensor=illums_C, axis=-1, keepdims=True) + EPS)
  ...
  illums_C_pred = (illums_C_pred + EPS) / (tf.norm(tensor=illums_C_pred, axis=-1, keepdims=True) + EPS)
  
  # This makes gray color in `images_AC to be `illums_C` instead of `illums_C_pred`.
  M_diag_C = tf.linalg.diag((illums_C + EPS) / (illums_C_pred + EPS)) # (b, 3, 3)
  ...
  images_BA = tf.reshape(tf.matmul(tf.reshape(images_B, (b, -1, 3))[...,::-1], M_BA), (b, tf.shape(input=images)[1], tf.shape(input=images)[2], 3))[...,::-1] # Pos
  images_AC = tf.reshape(tf.matmul(tf.reshape(images_A, (b, -1, 3))[...,::-1], M_AC), (b, tf.shape(input=images)[1], tf.shape(input=images)[2], 3))[...,::-1] # Hard Neg
  images_BC = tf.reshape(tf.matmul(tf.reshape(images_B, (b, -1, 3))[...,::-1], M_BC), (b, tf.shape(input=images)[1], tf.shape(input=images)[2], 3))[...,::-1] # Easy Neg
  
  # Align brightness to anchor images (since applying M will increase or decrease brightness)
  norms_A = tf.reduce_mean(input_tensor=images_A, axis=[1,2,3], keepdims=True) # (b, 1, 1, 1)
  images_BA = images_BA / tf.reduce_mean(input_tensor=images_BA, axis=[1,2,3], keepdims=True) * norms_A
  images_AC = images_AC / tf.reduce_mean(input_tensor=images_AC, axis=[1,2,3], keepdims=True) * norms_A
  images_BC = images_BC / tf.reduce_mean(input_tensor=images_BC, axis=[1,2,3], keepdims=True) * norms_A
