...
def angular_loss(tensor1, tensor2, return_dot_product=False, average=False):
    ...
    dot_prod = tf.reduce_sum(input_tensor=normalized_tensor1 * normalized_tensor2, axis=-1)
    ...
    if not return_dot_product:
        return tf.reduce_mean(input_tensor=angle) if average else angle
    else:
        return (tf.reduce_mean(input_tensor=angle), tf.reduce_mean(input_tensor=dot_prod)) if average else angle, dot_prod

def nce_loss(a_features, p_features, n_features, num_negatives, average=True):
    # n_features (N * b, c)
    TEMPERATURE = 0.87
    TRAINING_BATCH_SIZE = tf.shape(input=a_features)[0]
    ...
    losses = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(onehot_labels), logits=scores)
    loss = tf.reduce_mean(input_tensor=losses) if average else losses
